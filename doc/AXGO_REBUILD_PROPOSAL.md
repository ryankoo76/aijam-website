...
### 4-1. 등록 플로우 — aijam-us.com과 동일

AXGO의 등록 플로우는 aijam-us.com과 **완전히 동일**합니다.

```
등록 → 결제 → 제출(온라인 프로젝트 정보 입력)
```

`submission_status` 컬럼 값도 그대로 재사용:
```
'registered'  ← 등록 완료
'paid'        ← 결제 완료
'submitted'   ← 제출 완료
...
코드 수정 없이 환경변수와 콘텐츠 교체만으로 동작합니다.
...
| 7 | 결제 독촉 | 결제 독촉 (registered 상태 미결제자) |

### 4-4. 물리적 행사 특성상 추가 필요 기능

| 기능 | 필요도 | 비고 |
|------|--------|------|
| 행사 안내 이메일 (7/1경 발송) | 🔴 필수 | 부스 번호, 일정표, Hongdae 지도 포함 |
...
| 등록 폼 필드 조정 (Track A/B/C/D 추가) | 🟡 보통 | 1–2시간 | 카테고리 옵션 변경 |
| 제출 폼 콘텐츠 조정 (현장 행사 맞춤) | 🟡 보통 | 2–3시간 | 필드 라벨·옵션 교체 |
| Supabase 테이블 생성 | 🟢 쉬움 | 1시간 | SQL 실행 |
| Stripe + Resend + Vercel 세팅 | 🟡 보통 | 2시간 | 계정/환경변수 설정 |
| 전체 테스트 | 🟡 보통 | 2–3시간 | E2E 플로우 확인 |

**총 예상 작업 시간: 약 16–22시간 (2–3일)**

> 비교: axgo.org를 완전히 처음부터 만들면 30–60일 소요  
> **등록 플로우가 동일하므로 코드 재사용률 약 90%, 시간 단축 효과 극대화**
...
Phase 3 (Day 3) — 등록 & 결제
  ├─ 등록 폼에 Track 선택 필드 추가 (A/B/C/D)
  └─ Stripe 결제 금액/상품명 변경 (axgo용)

Phase 4 (Day 4) — 제출 폼 & 이메일
  ├─ 제출 폼 필드 라벨·옵션 AXGO 맞춤 교체
...
| 행사 특성 (물리 전시) | 온라인 제출과 달리 부스 번호 배정 등 운영 요소 존재 | 초기 버전에서는 단순화, v2에서 확장 |
